...
def generate_ellipsoid_clusters(distance, n_samples=100, cluster_std=0.5):
    ...
    shift_vector = distance * direction        # Calculate shift vector
    X2 += shift_vector
    ...
    for i, distance in enumerate(shift_distances, 1):
        ...
        for level, alpha in zip(contour_levels, alphas):
            ...
            if level == 0.7:
                ...
                if len(paths_class1) > 0 and len(paths_class0) > 0:
                    vertices_class1 = np.concatenate([p.vertices for p in paths_class1])
                    vertices_class0 = np.concatenate([p.vertices for p in paths_class0])
